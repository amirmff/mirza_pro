...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($step == 1) {
        ...
        try {
            // Update config.php (override placeholders fully)
            $config_content = "<?php\n";
            $config_content .= "// Generated by setup wizard\n";
            $config_content .= "\\$dbname = '" . addslashes($_SESSION['db_name']) . "';\n";
            $config_content .= "\\$usernamedb = '" . addslashes($_SESSION['db_user']) . "';\n";
            $config_content .= "\\$passworddb = '" . addslashes($_SESSION['db_pass']) . "';\n\n";
            $config_content .= "\\$connect = mysqli_connect(\\"localhost\\", \\$usernamedb, \\$passworddb, \\$dbname);\n";
            $config_content .= "if (!\\$connect || \\$connect->connect_error) { die(\\"Database connection error.\\"); }\n";
            ...
            $config_content .= "\\$APIKEY = '" . addslashes($_SESSION['bot_token']) . "';\n";
            $config_content .= "\\$adminnumber = '" . addslashes($_SESSION['admin_id']) . "';\n";
            $config_content .= "\\$domainhosts = '" . addslashes($_SESSION['domain']) . "';\n";
